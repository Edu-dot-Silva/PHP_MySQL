...
if (!empty($_GET['ordem']) && is_array($_GET['ordem'])) {
    ...
    foreach ($_GET['ordem'] as $ordem) {
        switch ($ordem) {
            case 'preco_asc': $ordens[] = "preco ASC"; break;
            case 'preco_desc': $ordens[] = "preco DESC"; break;
            case 'nome_asc': $ordens[] = "nome ASC"; break;
            case 'nome_desc': $ordens[] = "nome DESC"; break;
        }
    }
...
<body>
    ...
    <div style="display: flex; flex-wrap: wrap; gap: 24px;">
        <?php if ($cds && $cds->num_rows > 0): ?>
            <?php while($p = $cds->fetch_assoc()): ?>
                <div style="border:1px solid #ccc; padding:16px; width:200px; cursor:pointer;">
                    <?php if (!empty($p['imagem_url'])): ?>
                        ...
                        <img src="../../AdminHeavyWords/assets/img/produtos/cd/<?php echo basename($p['imagem_url']); ?>" alt="<?php echo htmlspecialchars($p['nome']); ?>" style="max-width:100%;max-height:120px;display:block;margin-bottom:8px;">
                        </a>
                    <?php endif; ?>
